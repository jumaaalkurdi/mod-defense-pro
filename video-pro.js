...
function svgThumb(seed){
  var c = '#c9a34e';
  var d = Math.abs(seed) % 6;

  var defs = '<defs>'
    + '<linearGradient id="bgv' + d + '" x1="0" y1="0" x2="1" y2="1">'
    + '<stop offset="0" stop-color="#1a2210"/>'
    + '<stop offset="0.5" stop-color="#0f1408"/>'
    + '<stop offset="1" stop-color="#06070a"/>'
    + '</linearGradient>'
    + '<radialGradient id="glowv' + d + '" cx="50%" cy="50%" r="60%">'
    + '<stop offset="0" stop-color="' + c + '" stop-opacity="0.14"/>'
    + '<stop offset="1" stop-color="' + c + '" stop-opacity="0"/>'
    + '</radialGradient>'
    + '</defs>';

  var bg = '<rect width="800" height="450" fill="url(#bgv' + d + ')"/>';
  bg += '<rect width="800" height="450" fill="url(#glowv' + d + ')"/>';

  var pattern = '';

  /* 0 - شبكة عسكرية */
  if(d === 0){
    pattern = '<g stroke="' + c + '" stroke-width="0.5" opacity="0.13" fill="none">';
    for(var x = 0; x <= 800; x += 50) pattern += '<line x1="' + x + '" y1="0" x2="' + x + '" y2="450"/>';
    for(var y = 0; y <= 450; y += 50) pattern += '<line x1="0" y1="' + y + '" x2="800" y2="' + y + '"/>';
    pattern += '</g>';
    pattern += '<g fill="' + c + '" opacity="0.5">';
    pattern += '<circle cx="150" cy="100" r="2"/><circle cx="640" cy="340" r="2"/>';
    pattern += '<circle cx="400" cy="225" r="1.5"/><circle cx="250" cy="380" r="1.5"/>';
    pattern += '</g>';
  }

  /* 1 - دوائر متداخلة */
  if(d === 1){
    pattern = '<g stroke="' + c + '" stroke-width="1" opacity="0.11" fill="none">';
    ...
    pattern += '</g>';
    pattern += '<g stroke="' + c + '" stroke-width="0.6" opacity="0.07" fill="none">';
  ...
  /* 2 - خطوط مائلة */
  if(d === 2){
    pattern = '<g stroke="' + c + '" stroke-width="1" opacity="0.09" fill="none">';
    for(var k = -450; k < 800; k += 35){
  ...
  /* 3 - جبال */
  if(d === 3){
    pattern = '<g fill="none" stroke="' + c + '" opacity="0.14">';
    pattern += '<path d="M0 350 L150 250 L280 320 L450 180 L600 280 L750 220 L800 250" stroke-width="1.5"/>';
  ...
  /* 5 - سداسي */
  if(d === 5){
    pattern = '<g fill="none" stroke="' + c + '" stroke-width="0.6" opacity="0.1">';
  ...
  /* خط توهج */
  var line = '<rect x="0" y="224" width="800" height="2" fill="' + c + '" opacity="0.25"/>';

  /* شعار صغير */
  var mark = '<g transform="translate(730,35)" opacity="0.25">'
    + '<path d="M0 0 L20 8 L20 24 C20 36 14 44 10 48 C6 44 0 36 0 24 Z" fill="none" stroke="' + c + '" stroke-width="1.5"/>'
    + '</g>';

  /* إطار */
  var frame = '<rect x="0.5" y="0.5" width="799" height="449" fill="none" stroke="' + c + '" stroke-width="1" opacity="0.22"/>';
